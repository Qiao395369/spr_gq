...
import attr
from vmcnet.gaoqiao.lapnet import curvature_tags_and_blocks
from vmcnet.gaoqiao.lapnet import network_blocks
from vmcnet.gaoqiao.lapnet import sto
...
def make_isotropic_envelope(is_abs=False) -> Envelope:
  ...
  def apply(*, ae: jnp.ndarray, r_ae: jnp.ndarray, r_ee: jnp.ndarray,
    ...
    del ae, r_ee  # unused
    if is_abs:
        return jnp.sum(jnp.exp(-jnp.abs(r_ae * sigma)) * pi, axis=1)
    else:
...
def get_envelope(
  ...
  envelope_builders = {
      ...
      EnvelopeLabel.ABS_ISOTROPIC: lambda: make_isotropic_envelope(is_abs=True),
