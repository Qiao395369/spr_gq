...
def init_RHF_orbital_shaping(
    key: chex.PRNGKey,
    input_dim: int,
    nspin_orbitals: Sequence[int],
    bias_orbitals: bool,
) -> Sequence[Param]:
  assert nspin_orbitals[0]==nspin_orbitals[1] and nspin_orbitals[0]%2==0
  num_otbitals=nspin_orbitals[0]//2
  orbitals = []
  for ii in range(2):
    key, subkey = jax.random.split(key)
    orbitals.append(
...
def construct_input_features(
  ...
  ea = jnp.reshape(pos, [-1, 1, ndim]) - atoms[None, ...]   #(ne,1,3)-(1,na,3)->(ne,na,3)
  ae = jnp.transpose(-ea, [1, 0, 2])   #(ne,na,3)->(na,ne,3)
  ...
  return ae,ea, ee, aa, r_ae,r_ea, r_ee[..., None], r_aa
...
def make_fermi_net_model_ef_shrd_sym_ablation(
  ...
  def init(
    ...
    key, subkey = jax.random.split(key)
    params['one_reshp'] = network_blocks.init_linear_layer(subkey, num_two_features + dim_1_append, hidden_dims[0][0])
  ...
  def apply(
    ...
    h1_=e2_features[:nele,...] #(ne,np,nf_two)
    h1 = jnp.mean(h1_,axis=1)  #(ne,np,nf_two)->(ne,nf_two)
...
def make_fermi_net_model_ef_shrd_sym_ablation_zero(
  ...
  """
  Controlled ablation for equal-footing model.

  Difference from make_fermi_net_model_ef_shrd_sym:
    - Keep the same full electron+nucleus representation.
    - Keep the same dimensions, projections, residuals, activation, and apply flow.
    - Only zero out nuclear one-particle summaries and nuclear-source pair summaries
      inside construct_symmetric_features_conv.

  This tests whether nuclear pooled summaries in the shared backbone are important,
  without turning the model into a different electron-only architecture.
  """
  ...
  def init(key):
    ...
    # IMPORTANT:
    # Keep exactly the same input dimensions as the full equal-footing model.
    # Do not reduce these dimensions in the ablation.
    nfeatures = lambda out1, out2: (nchannels + 2) * out1 + (nchannels + 1) * out2
  ...
  def construct_symmetric_features_conv(
    ...
    """
    Fair nuclear-summary ablation.

    Full model does:
      g1 = [mean nuclear h1] + [mean spin-up h1] + [mean spin-down h1]
      g2 = [mean nuclear-source pair messages] + [mean spin-up messages] + [mean spin-down messages]

    This ablation does:
      g1 = [zero nuclear summary] + [mean spin-up h1] + [mean spin-down h1]
      g2 = [zero nuclear-source summary] + [mean spin-up messages] + [mean spin-down messages]

    All dimensions are preserved.
    """
...
def make_fermi_net_model_ef_shrd_sym_stopgrad(
    natom,
    ndim,
    nspins,
    feature_layer,
  ...
  def construct_symmetric_features_conv(
    ...
    g2_nuc = jnp.mean(h2_below, axis=0)
    g2_nuc = jax.lax.stop_gradient(g2_nuc)
    g2 = [g2_nuc] + [jnp.mean(h, axis=0) for h in jnp.split(h2_upper, spin_partitions, axis=0) if h.size > 0]
    g2 = jnp.concatenate(g2, axis=1)

    g1_nuc = jnp.mean(hz, axis=0, keepdims=1)
    g1_nuc = jax.lax.stop_gradient(g1_nuc)
    g1 = [g1_nuc] + [jnp.mean(h, axis=0, keepdims=1) for h in jnp.split(hi, spin_partitions, axis=0) if h.size > 0]
    g1 = jnp.concatenate(g1, axis=1)
    g1 = jnp.tile(g1,[h1.shape[0],1])
    return jnp.concatenate([h1,g1,g2], axis=1)
...
def make_fermi_net_model_ef_shrd_sym_alpha(
    natom,
    ndim,
    nspins,
    feature_layer,
  ...
  def construct_symmetric_features_conv(
    ...
    spin_partitions = network_blocks.array_partitions(nspins)
    # [(nele+na) x nf2, (nele+na) x nf2, (nele+na) x nf2]
    g2 = [alpha * jnp.mean(h2_below, axis=0)]+[jnp.mean(h, axis=0) for h in jnp.split(h2_upper, spin_partitions, axis=0) if h.size > 0]
    g2 = jnp.concatenate(g2, axis=1)

    # [1 x nf1, 1 x nf1, 1 x nf1]
    g1 = [alpha * jnp.mean(hz, axis=0, keepdims=1)]+[jnp.mean(h, axis=0, keepdims=1) for h in jnp.split(hi, spin_partitions, axis=0) if h.size > 0]
    g1 = jnp.concatenate(g1, axis=1)
    g1 = jnp.tile(g1, [h1.shape[0], 1])
...
def make_fermi_net_model_ef_shrd_sym_proj(
    natom,
    ndim,
    nspins,
    feature_layer,
  ...
  def init(
    ...
    for ii in range(len(params['two'])):
      if dim_proj_1_in[ii] != dim_proj_1_out[ii]:
        key, subkey = jax.random.split(key)
        params['proj'].append(network_blocks.init_linear_layer( subkey, dim_proj_1_in[ii], dim_proj_1_out[ii],include_bias=False ))
        #[64,64,64] 
        # |  |  |
        # |  |  |
        # V  V  V 
    ...
    if dim_proj_0_in != dim_proj_0_out:
      key, subkey = jax.random.split(key)
      params['proj_0'] = network_blocks.init_linear_layer(subkey, dim_proj_0_in, dim_proj_0_out,include_bias=False )
      #7-->4
    else:
      params['proj_0'] = None
  ...
  def construct_symmetric_features_conv(
    ...
    spin_partitions = network_blocks.array_partitions(nspins)
    # [(nele+na) x nf2, (nele+na) x nf2, (nele+na) x nf2]
    g2 = [jnp.mean(h2_below, axis=0)]+[jnp.mean(h, axis=0) for h in jnp.split(h2_upper, spin_partitions, axis=0) if h.size > 0]
    g2 = jnp.concatenate(g2, axis=1)

    # [1 x nf1, 1 x nf1, 1 x nf1]
    g1 = [jnp.mean(hz, axis=0, keepdims=1)]+[jnp.mean(h, axis=0, keepdims=1) for h in jnp.split(hi, spin_partitions, axis=0) if h.size > 0]
    g1 = jnp.concatenate(g1, axis=1)
    g1 = jnp.tile(g1, [h1.shape[0], 1])
...
def make_fermi_net_model_ef_shrd_sym_extreme(
    natom,
    ndim,
    nspins,
    feature_layer,
...
def make_fermi_net_model_ef_shrd_sym_old(
    natom,
    ndim,
    nspins,
    feature_layer,
...
def make_fermi_net_model_zinv_shrd(
  ...
  def construct_symmetric_features_conv(
    ...
    hzixhz = hzi * phz[:,None,:]  #(nz,nele,nf)*(nz,1,nf)->(nz,nele,nf)  p_hz *(pointwise product) hzi
    ...
    # nf_one * 2 + nf_two * 2
      ...
      # nele x nfiz
      hzi_i = [jnp.mean(hzixhz, axis=0)]  #\Sigma_z  p_hz *(pointwise product) hzi  (nz,nele,nf)->(nele,nf)
      # [nele x nfij, nele x nfij]
  ...
  def apply(
    ...
    for i in range(len(params['two'])):
      ...
      if i == 0:
        ...
        # print("shrd hi_in ", hi_in)
        # print("shrd hz_in ", hz_in)
        hz_in, hi_in = construct_symmetric_features_conv(
          hz, hi,hzi, hiz, hij, hyz, nspins,
      ...
      if i == 0 or not code_only_first:
        [hij, hiz, hyz,hzi] = [jnp.concatenate([ii,jj], axis=-1) for ii,jj in zip([hij, hiz, hyz,hzi], [aij, aiz, ayz,azi])]
        # if do_attn_1:
...
def fermi_net_orbitals_part1(
  ...
  ae,ea, ee, aa, r_ae,r_ea, r_ee, r_aa = construct_input_features(pos, atoms, do_aa=options.do_aa)
  hae,hea, hee, haa = options.feature_layer.apply(ae=ae, r_ae=r_ae,ea=ea,r_ea=r_ea, ee=ee, r_ee=r_ee, aa=aa, r_aa=r_aa, **params['input'])
  model_h_to_orbitals, hz = options.ferminet_model.apply(params, hae,hea, hee, aa_features=haa, pos=pos,)
...
def fermi_net_orbitals_part2(
  ...
  # Create orbitals.
  if options.RHF:
    orbitals_of_up=jnp.concatenate([network_blocks.linear_layer(h_to_orbitals[0], **p)for p in params['RHF_orbital']], axis=-1)
    orbitals_of_dn=jnp.concatenate([network_blocks.linear_layer(h_to_orbitals[1], **p)for p in params['RHF_orbital'][::-1]],axis=-1)
    orbitals=[orbitals_of_up,orbitals_of_dn]
  else:
