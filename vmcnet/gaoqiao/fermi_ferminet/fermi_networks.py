...
class BaseNetworkOptions:
  ...
  envelope: fermi_envelopes.Envelope = attr.ib(
...
def _split_spin_pairs(
  ...
  up_up, up_down, down_up, down_down = fermi_network_blocks.split_into_blocks(
      arr, nspins
  )
...
def _combine_spin_pairs(
  ...
  same_pair_partitions = fermi_network_blocks.array_partitions(nsame_pairs)
...
def make_schnet_convolution(
  ...
  def init(
    ...
    h_one_kernel = fermi_network_blocks.init_linear_layer(
        key_one, in_dim=dims_one, out_dim=embedding_dim, include_bias=False
    )
    h_two_kernels = []
    for i in range(nchannels):
      h_two_kernels.append(
          fermi_network_blocks.init_linear_layer(
              key_two[i],
              in_dim=dims_two,
  ...
  def apply(
    ...
    h_one_embedding = fermi_network_blocks.linear_layer(h_one, params['single'])
    h_two_embeddings = [
        fermi_network_blocks.linear_layer(h_two_channel, layer_param)
...
def make_schnet_electron_nuclear_convolution() -> ...:
  ...
  def init(
    ...
    return {
        'electron_ion_embedding': fermi_network_blocks.init_linear_layer(
            key1,
            in_dim=electron_nuclear_dim,
            out_dim=embedding_dim,
            include_bias=False,
        )['w'],
        'ion_embedding': fermi_network_blocks.init_linear_layer(
...
def make_fermi_net_layers(
  ...
  def init(key: chex.PRNGKey) -> Tuple[int, ParamTree]:
    ...
    if options.nuclear_embedding_dim:
      ...
      params['nuclear'] = fermi_network_blocks.init_linear_layer(
          nuclear_key,
          in_dim=1,
    ...
    for i in range(len(options.hidden_dims)):
      ...
      layer_params['single'] = fermi_network_blocks.init_linear_layer(
          single_key,
          in_dim=dims_one_in,
      ...
      if i < len(options.hidden_dims) - 1 or options.use_last_layer:
        ...
        if options.electron_nuclear_aux_dims:
          layer_params['electron_ion'] = fermi_network_blocks.init_linear_layer(
              aux_key,
  ...
  def apply_layer(
    ...
    # Execute next layer.
    h_one_next = jnp.tanh(
        fermi_network_blocks.linear_layer(h_one_in, **params['single'])
    )
    ...
    if 'double' in params:
      if options.separate_spin_channels:
        ...
        params_double = [params['double']]
      h_two_next = [
          jnp.tanh(fermi_network_blocks.linear_layer(prev, **param))
    ...
    if h_elec_ion is not None and 'electron_ion' in params:
      h_elec_ion = fermi_network_blocks.linear_layer(
...
def make_orbitals(
  ...
  def init(key: chex.PRNGKey) -> ParamTree:
    ...
    elif options.envelope.apply_type == fermi_envelopes.EnvelopeType.PRE_DETERMINANT:
      # Applied to orbitals.
      if options.complex_output:
  ...
  def apply(
    ...
    h_to_orbitals = jnp.split(
        h_to_orbitals, fermi_network_blocks.array_partitions(nspins), axis=0
    )
    # Drop unoccupied spin channels
    h_to_orbitals = [h for h, spin in zip(h_to_orbitals, nspins) if spin > 0]
    active_spin_channels = [spin for spin in nspins if spin > 0]
    active_spin_partitions = fermi_network_blocks.array_partitions(
        active_spin_channels
    )
    # Create orbitals.
    orbitals = [
        fermi_network_blocks.linear_layer(h, **p)
    ...
    if options.envelope.apply_type == fermi_envelopes.EnvelopeType.PRE_DETERMINANT:
