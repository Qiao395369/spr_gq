...
def make_position_amplitude_gaussian_proposal(
    ...
    def proposal_fn(params: P, data: PositionAmplitudeData, key: PRNGKey):
        ...
        proposed_position, key = metropolis.gaussian_proposal(
            data["walker_data"]["elec_position"], std_move, key
        )
        atoms_position = data["atoms_position"]
...
def make_position_amplitude_metropolis_symmetric_acceptance(
    ...
    def acceptance_fn(
        ...
        return metropolis.metropolis_symmetric_acceptance(
            ...
            logabs=logabs,
        ) #a (B,) array, elements are the accept ratio 
...
def _scatter_to_original_order_data(x0,x1,idx):
    n0 = x0.shape[0]
    n1 = x1.shape[0]
    walker_num = n0 + n1
...
def make_reform_data_and_metrics_fn(apply_pmap):
    ...
    def _reform_data_and_metrics(data, rest_data, metrics, idx, variance1, multi_energy1, accept_ratio0, accept_ratio1):
        scatter_to_original_order_data = functools.partial(_scatter_to_original_order_data, idx=idx)
        data_out = jax.tree_util.tree_map(scatter_to_original_order_data, data, rest_data)
        
        multi_energy0 = metrics["multi_energy"]    # 形状 [n0, ...]
        multi_energy = scatter_to_original_order_data(multi_energy0, multi_energy1)

        variance0 = metrics["multi_variance"]
        multi_variance = scatter_to_original_order_data(variance0,  variance1)

        accept_ratio = scatter_to_original_order_data(accept_ratio0, accept_ratio1)

        new_metrics = dict(metrics)
        new_metrics["multi_energy"] = multi_energy
        new_metrics["multi_variance"] = multi_variance
        new_metrics["accept_ratio"] = accept_ratio
        new_metrics["accept_ratio_mean"] = utils.distribute.pmean_if_pmap(jnp.mean(accept_ratio))
        new_metrics["std_move"] = data_out["move_metadata"]["std_move"]
        new_metrics["move_acceptance_sum"] = data_out["move_metadata"]["move_acceptance_sum"]
        new_metrics["moves_since_update"] = data_out["move_metadata"]["moves_since_update"]

        return data_out, new_metrics
    
    if apply_pmap:
        reform_data_and_metrics = jax.pmap(_reform_data_and_metrics,axis_name=PMAP_AXIS_NAME,)
    else:
        reform_data_and_metrics = jax.jit(_reform_data_and_metrics,)
...
def init_dummy_metrics_for_downsample(is_pmapped: bool):
    def aaa(atoms):
        ...
        accept_ratio = jnp.zeros((W,), dtype=atoms.dtype)        # (W,)
        return multi_variance, multi_energy, accept_ratio
